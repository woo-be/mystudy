...
public class CalcServer3 {
  ...
  // 특정 코드 실행을 main() 메서드 호출과 분리해서 별도로 실행시키기
  static class OtherExcecution extends Thread {

    Socket socket;

    public OtherExcecution(Socket socket) {
      this.socket = socket;
    }
  ...
  public static void main(String[] args) throws Exception {
    ...
    while (true) {
      ...
      OtherExcecution 클라이언트요청처리 = new OtherExcecution(socket);
      클라이언트요청처리.start();
      // run() 메서드의 실행과 분리하여 별도로 run() 메서드를 호출한 후 즉시 리턴한다.
    }
    // ss.close();
  }
